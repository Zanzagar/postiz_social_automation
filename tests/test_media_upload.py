...
class TestMediaUpload:
    ...
    def test_upload_creates_catalog_entry(
        self, mock_postiz, mock_tags, client, auth_headers, tmp_path
    ):
        mock_postiz.return_value = None  # Postiz upload optional
        mock_tags.return_value = [{"tag": "outdoor", "confidence": 0.9}]
    ...
    @patch("api.routes.media._classify_media_tags")
    @patch("api.routes.media._upload_to_postiz")
    def test_upload_saves_tags(self, mock_postiz, mock_tags, client, auth_headers):
        mock_postiz.return_value = None
        mock_tags.return_value = [
    ...
    @patch("api.routes.media._classify_media_tags")
    @patch("api.routes.media._upload_to_postiz")
    def test_postiz_failure_still_catalogs(self, mock_postiz, mock_tags, client, auth_headers):
...
class TestMediaImportUrl:
    ...
    def test_import_url_creates_catalog(
        self, mock_download, mock_postiz, mock_tags, client, auth_headers
    ):
        img = _make_test_image()
        mock_download.return_value = (img.read(), "image/jpeg", "photo.jpg")
        mock_postiz.return_value = None
        mock_tags.return_value = [{"tag": "landscape", "confidence": 0.85}]
